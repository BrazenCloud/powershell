# How-To
This document describes how to develop for `BrazenCloud`.

## Building `BrazenCloud`
...
There are multiple scripts created for performing different actions for developing `BrazenCloud`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/BrazenCloud.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./BrazenCloud.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `BrazenCloud`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `BrazenCloud` into the session.
